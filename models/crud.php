...
//******************************
    ...
    public function editarUsuarioModel($datos,$tabla){
      $stmt = Conexion::conectar()->prepare("SELECT user_id, user_name,password,email,type,status FROM $tabla WHERE user_id=:user_id");
    ...
    public function actualizarUsuarioModel($datos,$tabla){
      $stmt = Conexion::conectar()->prepare("UPDATE $tabla SET user_name=:user_name, password=:password, email=:email, type=:type, status=:status WHERE user_id=:user_id");
      $stmt->bindParam(":user_id",$datos["id"],PDO::PARAM_INT);
      $stmt->bindParam(":user_name",$datos["usuario"],PDO::PARAM_STR);
